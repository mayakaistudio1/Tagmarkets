...
### Core Features
- **Partner Digital Hub**: Personalized partner pages (e.g., `/p/dennis`) — state machine with 5 states (HERO → CHAT_OVERLAY → PRESENTATION_OVERLAY → ECOSYSTEM_OVERLAY + `/live` route). Shared message state between chat and presentation. Dennis AI chat uses real OpenAI (`gpt-4o-mini`) with SSE streaming via `POST /api/partner/dennis/chat`. Presentation (Deck v1): unified video background "Financial Data Universe" (`public/videos/financial_universe_bg.mp4`, 8s loop) + 10 scene images with particle silhouette character (`public/images/presentation/scene_01..10.png`) + glassmorphism text cards (rgba(12,6,25,0.72), blur 20px, radius 24px) + 2-3 contextual chips per slide (open chat with slide context injection). TOC popup. Slide 10 CTAs: "Записаться на созвон", "Начать по моей ссылке", "Задать вопрос Dennis AI", "Открыть JetUp Hub". Ecosystem overlay renders full `HomePage` component inside Dennis hub with back-to-Dennis navigation via `EcoNavContext`. See `docs/presentation_structure.md` for full details.
