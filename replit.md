...
- **Smart Invite System** (DEV ONLY): Partner invite tracking for Zoom events. Tables: `invite_events` (partnerName, partnerCu, zoomLink, title, eventDate, eventTime, inviteCode unique), `invite_guests` (name, email, phone, clickedZoom, clickedAt). Flow: admin creates event → gets unique `/invite/:code` URL → partner shares link → guest registers (name, email) → clicks "Join Zoom" (tracked) → redirected to Zoom. Telegram notifications on registration + event reports. Admin "Invites" tab for creating events, viewing guest lists, sending TG reports. API: `GET/POST /api/invite/:code`, `POST /api/invite/:code/register`, `POST /api/invite/:code/click`, admin CRUD under `/api/admin/invite-events`.
