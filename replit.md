...
- **Dennis Fast Start Promo**: Promo card on HomePage between hub cards and utility links. Form (Name, Email, CU Number) → POST `/api/partner/promo-apply` → success confirmation. Translations under `dp.*` keys. Admin tab "Promo" in `/admin` for viewing/approving/rejecting applications + CSV export. API: `GET /api/admin/promo-applications`, `PATCH /api/admin/promo-applications/:id/status`.
- **Admin Panel**: Password-protected (`/admin`) for managing chat logs, promotions, schedule events, speakers, and promo applications with CRUD operations. Includes rate-limited login. Banner export uses html2canvas to capture the rendered React preview component.
