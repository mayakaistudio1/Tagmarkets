                ...
                <div key={app.id} className="bg-white rounded-xl shadow-sm p-4" data-testid={`card-application-${app.id}`}>
                  ...
                  {app.status !== "verified" && app.status !== "rejected" && (
                    <div className="flex gap-2 pt-2 border-t border-gray-100">
                      ...
                      >
                        {actionLoading === app.id ? "Verifying..." : "Verify & Send Email"}
                      </button>
                      <button
                        onClick={() => handleReject(app.id)}
                        disabled={actionLoading === app.id}
                        className="px-4 flex items-center justify-center bg-red-50 text-red-600 py-2 rounded-lg text-sm font-medium hover:bg-red-100 transition-colors disabled:opacity-50"
